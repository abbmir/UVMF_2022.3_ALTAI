...
def run():
  ...
  for step in args.steps:
    ...
    if 'variables' in step_s and 'compile_files' in step_s['variables']:
      ...
      for file in step_s['variables']['compile_files'].split(' '):
        ...
        filelist_data.clear()
        ## Each file passed in will produce one or more .qf output file (replace .compile with .qf and add file association data) and results will be placed in $CWD
        #qf_file_prefix = os.path.join(os.path.abspath(os.getcwd()),os.path.splitext(os.path.split(file)[1])[0])
        qf_file_prefix = os.path.join(os.path.splitext(os.path.split(file)[1])[0])
  ...
  for command in commands:
    ...
    if args.dry_run:
      ...
      ret = None
      for c in cl:
        logger.debug("Last command returned status {}".format(c))
